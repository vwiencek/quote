...
4. The page answers with:
   - a random activity at the highest available intensity ≤ the slider, filtered by player (entries marked `both` apply to everyone) and by the selected keywords, prefixed with the addressed player's first name (`PLAYER_NAMES` at the top of `app.js`)
