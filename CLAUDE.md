...
- **Slider d'intensité 1–10 + bouton GO** (persisté, `intensityLevel`) : le tirage prend les gages dont l'intensité est **la plus proche** de la valeur choisie (exacte si dispo, sinon les plus proches de part et d'autre). Le GO est teinté à la couleur de l'intensité (dégradé vert→or→rouge, `intensityColor()`), et le statut affiche l'intensité réelle du gage si elle diffère du slider.
...
- **Design (refonte 2026-07-15)** : palette sombre bleu-nuit (`--bg #0a0e18`, carte `--card #111726`, fond en dégradé radial), police **Onest** auto-hébergée. Accent principal **rose** `--rose #ef5a78` (joueur actif, bouton Terminé, libellé de tour, chiffres du score). Toggles = pastilles rose quand actives ; chips de catégories = **cyan** `--cat`. Intensité : slider à piste dégradée vert→or→rouge (`--soft`/`--surprise`/`--hard`) + GO teinté à la couleur de la valeur. Anneau : couleur d'intensité du gage tiré / vert (fini). Joueur affiché en texte « ♂ Lui / ♀ Elle » (glyphes forcés en présentation texte via `&#65038;` — évite le rendu emoji de Firefox). Le badge est masqué (l'anneau porte l'info). Libellé de tour sous le sélecteur, seulement si « on alterne » : « Au tour de … ».
