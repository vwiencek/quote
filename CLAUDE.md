...
- Compte à rebours dans un **anneau de progression** (vert soft, rouge hard, vert fini). **Le minuteur ne démarre pas tout seul** : au tirage il s'affiche à l'arrêt (état `.ready`, anneau qui pulse, statut « Appuie sur le minuteur pour démarrer »), et **un tap sur l'anneau le démarre / met en pause / reprend** (même logique que le bouton ⏸/▶). Boutons ⏸ (pause/reprise), « + 1 min » (ajoute sans démarrer si à l'arrêt ; relance un gage terminé), **🔀 Passer** (tire un autre gage du même niveau sans passer par la protection anti-fausse-manip) et **Terminé ✔** (arrêt anticipé, même célébration que la fin du temps).
