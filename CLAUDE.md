...
- **Lecture vocale** : bouton **🔈 Lire le gage** (sous le texte) qui lit le gage à voix haute via l'API Web Speech (`speechSynthesis`, voix `fr-FR` si dispo) ; re-clic = ⏹ stop ; masqué si le navigateur ne gère pas la synthèse ; coupé automatiquement au tirage d'un nouveau gage. Aucune dépendance externe.
