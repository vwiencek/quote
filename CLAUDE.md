...
- Chips de keywords (uniques, générées depuis la feuille), multi-sélection, état sélectionné = rempli cyan + ✓ ; chip **« Tout / Rien »** en tête pour tout (dé)sélectionner d'un coup.
...
- Compte à rebours dans un **anneau de progression** (vert soft, rouge hard, vert fini) + boutons ⏸ (pause/reprise), « + 1 min », **🔀 Passer** (tire un autre gage du même niveau sans passer par la protection anti-fausse-manip) et **Terminé ✔** (arrêt anticipé, même célébration que la fin du temps).
...
- **Lecture vocale** : bouton **🔈 Lire le gage** (sous le texte) qui lit le gage à voix haute via l'API Web Speech (`speechSynthesis`). `pickFrenchVoice()` choisit la voix française la **plus naturelle** disponible (voix *enhanced/neural/Siri* et Google/Apple priorisées sur la voix « compacte » robotique ; repli local si hors-ligne) ; débit 0.98. Re-clic = ⏹ stop ; masqué si le navigateur ne gère pas la synthèse ; coupé au tirage d'un nouveau gage. Aucune dépendance externe. NB : sur iOS/macOS les meilleures voix (« améliorées ») doivent être téléchargées dans Réglages → Accessibilité → Contenu énoncé.
